...
#         # --- branch resolution logging ---
#         # branch_stats[entity_type][source] = count
#         self.branch_stats: dict[str, dict[str, int]] = defaultdict(lambda: defaultdict(int))
#         # per-entity audit trail, one row per FINAL resolved entity
#         self.entity_log: list[dict] = []
#         self.doc_count = 0

#     def _record_stats(self, doc_id, final_entities: list[Decision]) -> None:
#         """Called once per document, after overseer_node resolves final_entities.
#         Tags every resolved entity with which branch actually produced it."""
#         self.doc_count += 1
#         for d in final_entities:
#             if d.entity_type is None:
#                 continue
#             entity_type = getattr(d.entity_type, "value", d.entity_type)
#             source = getattr(d.source, "value", d.source)
...
#     def save_stats(self, output_dir: str = "output/branch_analysis") -> None:
#         """Dump aggregated branch-resolution distribution + raw entity log.
#         Call this once, after your driver script finishes looping over all docs."""
#         out = Path(output_dir)
#         out.mkdir(parents=True, exist_ok=True)
#         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
 
#         # 1) overall totals per branch (across all entity types)
...
#         json_path = out / f"branch_distribution_{timestamp}.json"
#         with open(json_path, "w") as f:
#             json.dump(summary, f, indent=2)
#         print(f"[stats] summary written to {json_path}")
 
#         # 2) flat CSV: entity_type, branch, count, pct_within_entity_type
#         csv_path = out / f"branch_distribution_{timestamp}.csv"
#         with open(csv_path, "w", newline="") as f:
#             writer = csv.writer(f)
#             writer.writerow(["entity_type", "branch", "count", "pct_within_entity_type"])
#             for entity_type, branch_counts in self.branch_stats.items():
#                 et_total = sum(branch_counts.values())
#                 for branch, count in branch_counts.items():
#                     pct = round(100 * count / et_total, 2) if et_total else 0.0
#                     writer.writerow([entity_type, branch, count, pct])
#             # overall row (entity_type = "ALL")
#             for branch, count in overall_totals.items():
#                 pct = round(100 * count / grand_total, 2) if grand_total else 0.0
#                 writer.writerow(["ALL", branch, count, pct])
#         print(f"[stats] CSV written to {csv_path}")
 
#         # 3) raw per-entity audit log (jsonl - one entity per line)
#         log_path = out / f"entity_log_{timestamp}.jsonl"
#         with open(log_path, "w") as f:
#             for row in self.entity_log:
#                 f.write(json.dumps(row) + "\n")
#         print(f"[stats] entity-level audit log written to {log_path} ({len(self.entity_log)} rows)")
...
class AgentWorkflow:
    ...
    def overseer_node(self, state : NERAgentState) -> dict:
        ...
        if not requery_spans:
            final_entities = [d for d in decisions if d.entity_type is not None]
            self._record_stats(doc_id, final_entities)
            return {
                "final_entities": final_entities,
                "requery_count" : 0
            }
